...
page = 1
while True:
    params = {"resource_type": "video", "type": "upload", "max_results": 500, "media_metadata": True}
    if next_cursor:
...
for res in all_resources:
    ...
    tracks.append({
        ...
        "title": title,
        "playlist": playlist,
        "duration": f"{mins}:{secs:02d}" if duration > 0 else "",
        "url": url,
        "created_at": res.get("created_at", "")
    })

# Deduplication intelligente : garder le fichier le plus recent par titre+playlist
from collections import defaultdict
groups = defaultdict(list)
for t in tracks:
    key = (t["title"].lower().strip(), t["playlist"])
    groups[key].append(t)

unique_tracks = []
for key, group in groups.items():
    # Garder le plus recent (created_at le plus grand)
    best = max(group, key=lambda x: x.get("created_at", ""))
    unique_tracks.append(best)

# Recalculer les playlists
playlists = {}
for t in unique_tracks:
    playlists[t["playlist"]] = playlists.get(t["playlist"], 0) + 1

unique_tracks.sort(key=lambda t: (t["playlist"], t["title"]))
output = {"total": len(unique_tracks), "playlists": sorted(playlists.keys()), "tracks": unique_tracks}
with open("catalogue.json", "w", encoding="utf-8") as f:
    json.dump(output, f, ensure_ascii=False, indent=2)
print(f"\n✅ {len(unique_tracks)} morceaux, {len(playlists)} playlists")
